...
static int qer_fill(struct qer *qer, struct gtp5g_dev *gtp, struct genl_info *info)
{
    ...
    config = trtcm_param_config(&(qer->dl_policer.param), qer->gbr.dl_high, qer->gbr.dl_low, qer->mbr.dl_high, qer->mbr.dl_low);
    config = trtcm_param_config(&(qer->ul_policer.param), qer->gbr.ul_high, qer->gbr.ul_low, qer->mbr.ul_high, qer->mbr.ul_low);
    config = trtcm_runtime_config(&(qer->dl_policer.param), &(qer->dl_policer.runtime));
    config = trtcm_runtime_config(&(qer->ul_policer.param), &(qer->ul_policer.runtime));
